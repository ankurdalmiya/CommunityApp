...
//  const Page = require('../support/pages/loginPage');
 ...
 Given(/^I am on the Login Page of Community Admin panel$/, async function() {
  ...
  When(/^I login using my "([^"]*)" and "([^"]*)"$/, async function(String email, String password) {
     await testController.typeText(commPage.commwebsite.typeemail(), email);
     await testController.typeText(commPage.commwebsite.typepass(), password);
