...
Then(/^Status should be "([^"]*)"$/, async function (text1) {
      ...
      await testcontroller.expect(Selector('#email').iText).eql('sss@gmail.com',true);
    console.log("true");
    
    await testController.expect(commPage.haleywebsite.firstSearchResult().innerText).contains(text);
