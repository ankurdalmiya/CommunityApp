...
//  const Page = require('../support/pages/loginPage');
 ...
 Given(/^I am on the Login Page of Community Admin panel$/, async function() {
  ...
  When(/^I login using {string} and {string}$/, async function(string, string2) {
     await testController.typeText(commPage.commwebsite.typeemail(), string);
     await testController.typeText(commPage.commwebsite.typepass(), string2);
